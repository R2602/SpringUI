...
const Feed = () => {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      const response = await axios.get(`https://spring-application-y1xb.onrender.com/posts/${query}`);
      setPost(response.data);
    };
    const fetchInitialPosts = async () => {
        const response = await axios.get(`https://spring-application-y1xb.onrender.com/posts`);
